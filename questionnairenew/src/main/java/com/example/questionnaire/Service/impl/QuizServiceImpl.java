...
//			System.out.println("title:"+qn.getTitle());
//			System.out.println("Description:"+qn.getDescription());
//			System.out.println("Start:"+qn.getStartDate());
//			System.out.println("End:"+qn.getEndDate());
	...
	@Override
	public QuizRes createQuestion(QuizReq req) {
		Questionnaire qn = req.getQuestionnaire();
		List<Question> quList = req.getQuestionList();
	...
	@Override
	public QuestionnaireRes searchQuestionnaireListEazy(String title, LocalDate startDate, LocalDate endDate) {
		title = StringUtils.hasText(title) ? title: "";
		startDate = startDate != null ? startDate : LocalDate.of(1971, 1, 1);
		endDate = endDate != null ? endDate : LocalDate.of(2099, 12, 31);
		List<Questionnaire> qnList = qnDao.findByTitleContainingAndStartDateGreaterThanEqualAndEndDateLessThanEqual(title, startDate, endDate);
...
//		qnList = qnDao.findByTitleContainingAndStartDateGreaterThanEqualAndEndDateLessThanEqual(title, startDate, endDate);
	...
	@Override
	public QuizRes getquestion(@RequestParam(name = "id", required = false) int id) {
	    if (id <= 0) {
	        return new QuizRes(null, null, RtnCode.QNPARAM_ERROR);
	    }

	    Optional<Questionnaire> qnOp = qnDao.findById(id);

	    if (qnOp.isEmpty()) {
	        return new QuizRes(null, null, RtnCode.ID_NOTFOUNT);
	    }
	    List<Question> questions = quDao.findAllByQuId(id);

//	    List<Integer> qIds = new ArrayList<Integer>();
	    System.out.println("qnOp:"+qnOp);
	    System.out.println("questions:"+questions);
//	    return new QuizRes(questions,RtnCode.SUCCESSFUL);
		return null;
	}
	
	 public List<Question> findQuestionsByQnId(int qnId) {
	        return quDao.findByQnId(qnId);
	    }

	 @Override
	    public QuizRes getQuestionsByQnId(int qnId) {
		 ...
		 if (qnId <= 0) {
	        ...
	        List<Question> questions = quDao.findByQnId(qnId);
	        return new QuizRes(qnOp,questions);
	    }
}
